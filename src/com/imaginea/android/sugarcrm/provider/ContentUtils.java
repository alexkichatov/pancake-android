...
public class ContentUtils {
	...
	private static String[] defaultSupportedModules = { Util.ACCOUNTS, Util.CONTACTS, Util.LEADS,
    ...
    public static List<String> getUserModules(Context context) {
        ...
        Cursor cursor = context.getContentResolver().query(Modules.CONTENT_URI, Modules.DETAILS_PROJECTION, null, null, ModuleColumns.MODULE_NAME);
        cursor.moveToFirst();
     ...
     * @return a boolean.
     */
    public static boolean isModuleAccessAvailable(Context context, String moduleName) {
        
        List<String> userModules = getUserModules(context);
     ...
     *             if any.
     */
    public static void setUserModules(Context context, List<String> moduleNames) throws SugarCrmException {
    
    	ContentResolver cr = context.getContentResolver();
   
        try {
            // TODO: This has to be removed and is only added to fix the DB insertion issue,
            int numOfUserModulesDeleted = cr.delete(Modules.CONTENT_URI, null, null);
            Log.d("ContentUtils", "number of user modules deleted: " + numOfUserModulesDeleted);
        ...
        try {
            for (String moduleName : moduleNamesSet) {
                if (moduleName != null && !moduleName.equals("")) {
                	ops.add(ContentProviderOperation.newInsert(Modules.CONTENT_URI).withValue(ModuleColumns.MODULE_NAME, moduleName).build());
                }
            }
    ...
    public static List<String> getModuleList(Context context) {
        List<String> userModules = getUserModules(context);
        List<String> supportedModules = Arrays.asList(defaultSupportedModules);
     ...
     */
    public static Map<String, ModuleField> getModuleFields(Context context, String moduleName) {
   
    	ContentResolver cr = context.getContentResolver();
        String selection = ModuleColumns.MODULE_NAME + "='" + moduleName + "'";
        Cursor cursor = cr.query(Modules.CONTENT_URI, Modules.DETAILS_PROJECTION, selection, null, null);
        cursor.moveToFirst();
        ...
        cursor = cr.query(ModuleFields_TableInfo.CONTENT_URI, ModuleFields_TableInfo.DETAILS_PROJECTION, selection, null, null);
        cursor.moveToFirst();
        for (int i = 0; i < cursor.getCount(); i++) {
            String name = cursor.getString(cursor.getColumnIndex(ModuleFieldColumns.NAME));
            ModuleField moduleField = new ModuleField(name, cursor.getString(cursor.getColumnIndex(ModuleFieldColumns.TYPE)), 
            		cursor.getString(cursor.getColumnIndex(ModuleFieldColumns.LABEL)),
            		cursor.getInt(cursor.getColumnIndex(ModuleFieldColumns.IS_REQUIRED)) == 1 ? true : false);
            cursor.moveToNext();
     ...
     */
    public static ModuleField getModuleField(Context context, String moduleName, String fieldName) {
